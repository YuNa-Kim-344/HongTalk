...
public class FriendInformPanel extends JFrame{
   ...
   public FriendInformPanel(String myId) {
      ...
      //화면 중앙에 올 Center패널 객체 생성
        CenterPanel center = new CenterPanel(myId);
...
class CenterPanel extends JPanel {
    ...
    private String myId; // 추가: 현재 사용자의 ID를 저장

    public CenterPanel(String myId) {
    	// 패널 레이아웃 및 스크롤 설정
        setLayout(new BorderLayout());
    ...
    // 친구  목록 업데이트
    protected void updateFriendPanel(Friendship friendship) {
    	try {
    		// user1과 myId가 같으면 데이터베이스에서 정보를 읽어와 화면에 출력
            if (friendship.getUser1().equals(myId)) {
                UserFriendDB.saveFriendship(friendship); // 이 부분은 삭제
                updateFriendListFromDatabase(); // 친구 목록을 데이터베이스에서 읽어와 화면 업데이트
            }
 ...
 // 화면 업데이트 메서드
    private void updateFriendListFromDatabase() {
        try {
            // user1과 myId가 같은 경우 데이터베이스에서 정보를 읽어옴
            List<Friendship> friendList = UserFriendDB.saveFriendship(friendship);

            friendInfoPanel.removeAll();
            for (Friendship friendship : friendList) {
                UserFriend userFriend = (UserFriend) friendship.getUser2();

                JLabel nameLabel = new JLabel(" " + userFriend.getUserName());
                nameLabel.setForeground(Color.BLACK);
                nameLabel.setFont(new Font("Dialog", Font.BOLD, 30));

                JLabel emailLabel = new JLabel(" " + userFriend.getUserEmail());
            ...
            // 빈 공간 패널 추가
            addEmptyPanels(maxRooms - friendList.size());
